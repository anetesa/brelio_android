...
class GetAppointmentsByDateUseCase @Inject constructor(
    private val appointmentRepository: AppointmentRepository,
) {
    suspend operator fun invoke(userId: String, date: LocalDate): Result<List<Appointment>> {
        if (userId.isBlank()) return Result.failure(IllegalArgumentException("User ID required"))
        return appointmentRepository.getAppointments(userId, date)
    }
}
